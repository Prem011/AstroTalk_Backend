...
exports.register =  async function(req, res, next){
    try{
        ...
        const existingUserEmail = await Login.findOne({email});


        if(existingUserEmail){
            return res.status(400).send({error: 'Email already exists, Please change your Email address'});
        }
        ...
        const newUser =  new Login({ username, email });
